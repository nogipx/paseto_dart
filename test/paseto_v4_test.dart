...
void main() {
  group('PASETO v4 Compatibility Tests', () {
    final testVectors = jsonDecode(
      File('test/vectors/v4.json').readAsStringSync(),
    ...
    for (final testCase in testCases) {
      ...
      test('Test Vector: $testName', () async {
        ...
        final isLocal = token.startsWith('v4.local.');
        final isPublic = token.startsWith('v4.public.');
...
Future<void> _testPublicToken({
  ...
  if (expectFail) {
    ...
    if (expectedPayload != null && testCase.containsKey('secret-key')) {
      ...
      // В v4 приватный ключ и публичный ключ идут в keypair
