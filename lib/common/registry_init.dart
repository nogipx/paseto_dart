...
class PasetoRegistryInitializer {
  ...
  static void initV2Local() {
    registry.register(StaticFactoryConfig(StreamCipher, 'ChaCha7539', () => ChaCha7539Engine()));
    registry.register(StaticFactoryConfig(Mac, 'POLY1305', () => Poly1305()));
  }

  // AES-CTR + HMAC для v3.local
  static void initV3Local() {
    registry.register(StaticFactoryConfig(BlockCipher, 'AES', () => AESEngine()));
    registry
        .register(StaticFactoryConfig(StreamCipher, 'CTR/AES', () => CTRStreamCipher(AESEngine())));
    registry.register(StaticFactoryConfig(Digest, 'SHA-384', () => SHA384Digest()));
    registry.register(StaticFactoryConfig(Mac, 'HMAC/SHA384', () => HMac(SHA384Digest(), 128)));
    registry.register(
        StaticFactoryConfig(KeyDerivator, 'HKDF/SHA-384', () => HKDFKeyDerivator(SHA384Digest())));
  }
  ...
  static void initV4Local() {
    ...
    registry.register(StaticFactoryConfig(Digest, 'SHA-512', () => SHA512Digest()));
    registry.register(
        StaticFactoryConfig(KeyDerivator, 'HKDF/SHA-512', () => HKDFKeyDerivator(SHA512Digest())));
    ...
    registry.register(StaticFactoryConfig(Digest, 'BLAKE2B', () => Blake2bDigest(digestSize: 32)));

    // Регистрируем Blake2b/256 как Mac для v4.local
    registry.register(
        StaticFactoryConfig(Mac, 'BLAKE2b/256', () => HMac(Blake2bDigest(digestSize: 32), 64)));
  }
  ...
  static void initV3Public() {
    registry.register(StaticFactoryConfig(Digest, 'SHA-384', () => SHA384Digest()));
    registry
        .register(StaticFactoryConfig(Signer, 'ECDSA/SHA-384', () => ECDSASigner(SHA384Digest())));
    ...
    registry.register(StaticFactoryConfig(SecureRandom, '', () => secureRandom));
    ...
    registry.register(StaticFactoryConfig(ECDomainParameters, 'P-384', () => ecParams));
  }
