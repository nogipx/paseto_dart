...
class K4SecretPw extends PaserkKey {
  ...
  static Future<K4SecretPw> wrap(
    ...
    if (key.rawBytes.length != K4SecretKey.keyLength) {
      throw ArgumentError(
        'Key must be exactly ${K4SecretKey.keyLength} bytes',
      );
    }
    ...
    return K4SecretPw(payload);
  }

  static Future<K4SecretKey> unwrap(
    String serialized,
    String password,
  ) async {
    if (!serialized.startsWith(PaserkKey.k4SecretPwPrefix)) {
      throw ArgumentError('Invalid k4.secret-pw format');
    }
